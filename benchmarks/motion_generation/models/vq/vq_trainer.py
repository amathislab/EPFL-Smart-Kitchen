...
class AETokenizerTrainer:
    ...
    def forward(self, batch_data):
        ...
        pred_motion = self.vq_model(motions)

        self.motions = motions
        self.pred_motion = pred_motion

        loss_rec = self.l1_criterion(pred_motion, motions)
        pred_local_pos = pred_motion[..., 3 : self.opt.joints_num * 3]
        local_pos = motions[..., 3 : self.opt.joints_num * 3]
    ...
    def train(
        self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval=None
    ):
        ...
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(
            self.opt_vq_model, milestones=self.opt.milestones, gamma=self.opt.gamma
        )
        ...
        print(
            ...
            % (len(train_loader), len(eval_val_loader))
        )
        ...
        while epoch < self.opt.max_epoch:
            ...
            self.logger.add_scalar(
                "Val/loss_rec", sum(val_loss_rec) / len(val_loss_rec), epoch
            )
            self.logger.add_scalar(
                "Val/loss_vel", sum(val_loss_vel) / len(val_loss_vel), epoch
            )

            print(
                "Validation Loss: %.5f Reconstruction: %.5f, Velocity: %.5f"
                % (
                    sum(val_loss) / len(val_loss),
                    sum(val_loss_rec) / len(val_loss),
                    sum(val_loss_vel) / len(val_loss),
                )
            )
            ...
            if epoch % self.opt.eval_every_e == 0:
                data = (
                    torch.cat([self.motions[:4], self.pred_motion[:4]], dim=0)
                    .detach()
                    .cpu()
                    .numpy()
                )
...
class RVQTokenizerTrainer:
    ...
    def forward(self, batch_data):
        ...
        loss = (
            loss_rec + self.opt.loss_vel * loss_explicit + self.opt.commit * loss_commit
        )
    ...
    def train(
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                loss, loss_rec, loss_vel, loss_commit, perplexity = self.forward(
                    batch_data
                )
                self.opt_vq_model.zero_grad()
                loss.backward()
            ...
            self.logger.add_scalar(
                "Val/loss_commit", sum(val_loss_commit) / len(val_loss), epoch
            )
            self.logger.add_scalar(
                "Val/loss_perplexity", sum(val_perpexity) / len(val_loss_rec), epoch
            )

            print(
                "Validation Loss: %.5f Reconstruction: %.5f, Velocity: %.5f, Commit: %.5f"
                % (
                    sum(val_loss) / len(val_loss),
                    sum(val_loss_rec) / len(val_loss),
                    sum(val_loss_vel) / len(val_loss),
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_dataloader):
                ...
                text_embs = self.encode_fnc(
                    self.text_encoder, conds, self.opt.device
                ).detach()
