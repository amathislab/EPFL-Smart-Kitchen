...
from utils.eval_t2m import (evaluation_mardm, evaluation_mask_transformer,
...
class MARDMTrainer:
    def __init__(self, args, t2m_transformer, vq_model, ema_mardm):
        self.opt = args
        self.t2m_transformer = t2m_transformer
        self.vq_model = vq_model
        self.ema_mardm = ema_mardm
    ...
    def forward(self, batch_data):
        ...
        conds, motion, m_lens, image = batch_data
        motion = motion.detach().float().to(self.device)
        m_lens = m_lens.detach().long().to(self.device)

        # (b, n, q)
        latent = self.vq_model.encode(motion)
        m_lens = m_lens // 4
        ...
        _loss = self.t2m_transformer.forward_loss(latent, conds, m_lens, image)
    ...
    def save(self, file_name, ep, total_it):
        ...
        ema_mardm_state_dict = self.ema_mardm.state_dict()
        clip_weights = [
        ...
        state = {
            ...
            "ema_mardm": ema_mardm_state_dict,
    ...
    def resume(self, model_dir):
        ...
        missing_keys, unexpected_keys = self.t2m_transformer.load_state_dict(
            checkpoint["t2m_transformer"], strict=False
        )
        missing_keys2, unexpected_keys2 = self.ema_mardm.load_state_dict(
            checkpoint["ema_mardm"], strict=False
        )
        ...
        try:
            self.opt_t2m_transformer.load_state_dict(
                checkpoint["opt_t2m_transformer"]
            )  # Optimizer
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch in enumerate(train_loader):
                ...
                update_ema(self.t2m_transformer, self.ema_mardm, 0.9999)
                ...
                if it % self.opt.log_every == 0:
                    ...
                    for tag, value in logs.items():
                        self.logger.add_scalar(
                            "Train/%s" % tag, value / self.opt.log_every, it
                        )
            ...
            (
                ...
                plot_func=plot_eval,
                save_ckpt=True,
                save_anim=False,  # (epoch%self.opt.eval_every_e==0)
            )
...
class MaskTransformerTrainer:
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        while epoch < self.opt.max_epoch:
            ...
            print(
                f"Validation loss:{np.mean(val_loss):.3f}, accuracy:{np.mean(val_acc):.3f}"
            )
            ...
            if np.mean(val_acc) > best_acc:
                print(
                    f"Improved accuracy from {best_acc:.02f} to {np.mean(val_acc)}!!!"
                )
...
class ResidualTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        ce_loss, pred_ids, acc = self.res_transformer(code_idx, conds, m_lens, image)
    ...
    def resume(self, model_dir):
        checkpoint = torch.load(model_dir, map_location=self.device)
        missing_keys, unexpected_keys = self.res_transformer.load_state_dict(
            checkpoint["res_transformer"], strict=False
        )
        assert len(unexpected_keys) == 0
        assert all([k.startswith("clip_model.") for k in missing_keys])

        try:
            self.opt_res_transformer.load_state_dict(
                checkpoint["opt_res_transformer"]
            )  # Optimizer
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        self.opt_res_transformer = optim.AdamW(
            self.res_transformer.parameters(),
            betas=(0.9, 0.99),
            lr=self.opt.lr,
            weight_decay=1e-5,
        )
        self.scheduler = optim.lr_scheduler.MultiStepLR(
            self.opt_res_transformer,
            milestones=self.opt.milestones,
            gamma=self.opt.gamma,
        )
