...
def scale_cosine_schedule(t, scale):
    return torch.clip(
        scale * torch.cos(t * math.pi * 0.5) + 1 - scale, min=0.0, max=1.0
    )
...
def cal_performance(pred, labels, ignore_index=None, smoothing=0.0, tk=1):
...
def cal_loss(pred, labels, ignore_index=None, smoothing=0.0):
