...
class MaskTransformer(nn.Module):
    def __init__(
        ...
        **kargs,
        ...
        self.output_process = OutputProcess_Bert(
            out_feats=opt.num_tokens, latent_dim=latent_dim
        )
    ...
    def load_and_freeze_token_emb(self, codebook):
        ...
        self.token_emb.weight = nn.Parameter(
            torch.cat(
                [codebook, torch.zeros(size=(2, d), device=codebook.device)], dim=0
            )
        )  # add two dummy tokens, 0 vectors
    ...
    def encode_image(self, raw_image):
        image = self.clip_model.encode_image(raw_image).float()
        return image

    def mask_cond(self, cond, force_mask=False):
        bs, modal_num, d = cond.shape
        if force_mask:
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        ...
        cond = self.cond_emb(cond).permute(1, 0, 2).contiguous()

        x = self.position_enc(x)
        xseq = torch.cat([cond, x], dim=0)  # (seqlen+1, b, latent_dim)

        padding_mask = torch.cat(
            [torch.zeros_like(padding_mask[:, 0:modal_num]), padding_mask], dim=1
        )  # (b, seqlen+1)
    ...
    def forward(self, ids, y, m_lens, image):
        """
        :param ids: (b, n)
        ...
        if self.cond_mode == "text":
            with torch.no_grad():
                text_cond_vector = self.encode_text(y)
                image_cond_vector = self.encode_image(image)
                cond_vector = torch.stack([text_cond_vector, image_cond_vector], dim=1)
        elif self.cond_mode == "action":
    ...
    def forward_with_cond_scale(
        ...
        if force_mask:
            return self.trans_forward(
                motion_ids, cond_vector, padding_mask, force_mask=True
            )
    ...
    def generate(
        ...
        if self.cond_mode == "text":
            with torch.no_grad():
                text_cond_vector = self.encode_text(conds)
                image_cond_vector = self.encode_image(image)
                cond_vector = torch.stack([text_cond_vector, image_cond_vector], dim=1)
    ...
    def edit(
        ...
        if edit_mask == None:
            ...
            scores = torch.where(edit_mask, 0.0, 1e5)
        else:
            mask_free = False
            edit_mask = edit_mask & ~padding_mask
            edit_len = edit_mask.sum(dim=-1)
        ...
        for timestep, steps_until_x0 in zip(
            ...
            # 0 < timestep < 1
            rand_mask_prob = (
                0.16 if mask_free else self.noise_schedule(timestep)
            )  # Tensor
            ...
            """
            # fix: the ratio regarding lengths, instead of seq_len
            num_token_masked = torch.round(rand_mask_prob * edit_len).clamp(
                min=1
            )  # (b, )
            ...
            # We do not want to re-mask the previously kept tokens, or pad tokens
            scores = (
                scores.masked_fill(~edit_mask, 1e5)
                if mask_free
                else scores.masked_fill(~is_mask, 1e5)
            )
    ...
    def edit_beta(
        ...
        probs_without_temperature = logits.softmax(dim=-1)  # (b, seqlen, ntoken)
        tokens[tokens == -1] = (
            0  # just to get through an error when index = -1 using gather
        )
        og_tokens_scores = probs_without_temperature.gather(
            2, tokens.unsqueeze(dim=-1)
        )  # (b, seqlen, 1)
...
class ResidualTransformer(nn.Module):
        ...
        **kargs,
        ...
        if shared_codebook:
            token_embed = nn.Parameter(
                torch.normal(mean=0, std=0.02, size=(_num_tokens, code_dim))
            )
            self.token_embed_weight = token_embed.expand(
                opt.num_quantizers - 1, _num_tokens, code_dim
            )
            if share_weight:
                self.output_proj_weight = self.token_embed_weight
                self.output_proj_bias = None
            else:
                output_proj = nn.Parameter(
                    torch.normal(mean=0, std=0.02, size=(_num_tokens, code_dim))
                )
                output_bias = nn.Parameter(torch.zeros(size=(_num_tokens,)))
                self.output_proj_weight = output_proj.expand(
                    opt.num_quantizers - 1, _num_tokens, code_dim
                )
                self.output_proj_bias = output_bias.expand(
                    opt.num_quantizers - 1, _num_tokens
                )
            ...
            if share_weight:
                self.embed_proj_shared_weight = nn.Parameter(
                    torch.normal(
                        mean=0,
                        std=0.02,
                        size=(opt.num_quantizers - 2, _num_tokens, code_dim),
                    )
                )
                ...
                output_proj_weight = torch.normal(
                    mean=0,
                    std=0.02,
                ...
                token_embed_weight = torch.normal(
                    mean=0,
                    std=0.02,
                    size=(opt.num_quantizers - 1, _num_tokens, code_dim),
                )
    ...
    def output_project(self, logits, qids):
        ...
        output_proj_bias = (
            None if self.output_proj_bias is None else self.output_proj_bias[qids]
        )
    ...
    def trans_forward(self, motion_codes, qids, cond, padding_mask, force_mask=False):
        ...
        padding_mask = torch.cat(
            [torch.zeros_like(padding_mask[:, 0 : modal_num + 1]), padding_mask], dim=1
        )  # (b, seqlen+2)
        output = self.seqTransEncoder(xseq, src_key_padding_mask=padding_mask)[
            modal_num + 1 :
        ]  # (seqlen, b, e)
    ...
    def forward_with_cond_scale(
        ...
        if force_mask:
            logits = self.trans_forward(
                motion_codes, qids, cond_vector, padding_mask, force_mask=True
            )
    ...
    def forward(self, all_indices, y, m_lens, image):
        """
        ...
        logits = self.trans_forward(
            history_sum, active_q_layers, cond_vector, ~non_pad_mask, force_mask
        )
        logits = self.output_project(logits, active_q_layers - 1)
        ce_loss, pred_id, acc = cal_performance(
            logits, active_indices, ignore_index=self.pad_id
        )
    ...
    def generate(
        ...
        history_sum = 0
        num_quant_layers = (
            self.opt.num_quantizers if num_res_layers == -1 else num_res_layers + 1
        )
