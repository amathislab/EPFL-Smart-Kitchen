...
from models.mardm.diffusions.diffusion import create_diffusion
from models.mardm.diffusions.transport import Sampler, create_transport
...
class DiffMLPs_SiT(nn.Module):
    def __init__(self, target_channels, z_channels, depth, width):
        ...
        self.train_diffusion = (
            create_transport()
        )  # default to linear, velocity prediction
...
def diffmlps_ddpm_xl(**kwargs):
    return DiffMLPs_DDPM(
        depth=16, width=1792, num_sampling_steps="50", learn_sigma=False, **kwargs
    )
